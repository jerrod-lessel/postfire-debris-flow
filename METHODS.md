...
| dNBR offset correction | [FILL: offset from Cell 8], from [FILL: unburned pixels from Cell 8] unburned pixels |
| Elevation | USGS 3DEP 10 m, [FILL: elevation range from Cell 10] |
| Soil | USDA STATSGO, [FILL: map unit count and KF range from Cell 14] |
...
- [FILL: moderate-or-high percentage from Cell 8] of the burn area is moderate or high severity by dNBR
- [FILL: steep-in-burn percentage from Cell 11] of the burn area is 23 degrees or steeper

**Basins**

171 drainage basins touching the fire, median size [FILL: from Cell 12] km², covering [FILL: burn area inside some basin, from Cell 13] of the burn area.
...
**Basins cover [FILL: Line coverage from Cell 13] to 87.6% of the burn area, not all of it.** Trunk canyons with more than 8 km² of contributing area are outside the model's calibration range and cannot serve as source basins. Those unassigned valley floors are exactly where debris flows travel and where damage occurs, so the map describes where flows initiate rather than where they end up.

**S barely varies.** STATSGO map units are 1 to 10 km² against a median basin of 0.38 km², so most basins sit inside a single map unit. Across the entire Bridge Fire, S spans 0.242 to 0.339. Its coefficient is the largest in the model, but with that little spread it acts closer to a constant offset than a discriminator. Almost all the between-basin variation in the results comes from T.
